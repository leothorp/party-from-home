...
export default function RoomList() {
    ...
    const getParticipants = (room: string) => {
        fetch(`/room/${room}/participants`).then(res =>{
            res.json().then(p => {
                participants.set(room, p.map((p: any) => p.identity));
    ...
    return (
        <ul>
            {
                rooms.map(room => {
                    const users = participants.get(room.id)?.map((p: string) => (
                        <li key={p}>{p}</li>
                    ));

                    return (
                        <li key={room.id}>
                            <a onClick={switchRoom(room.id)} href=''>{room.name}</a>
                            {users}
                        </li>
                    );
                })
